...
local function lsp_keymaps(bufnr)
	...
	keymap(bufnr, "n", "<F12>", '<cmd>lua require("telescope.builtin").lsp_definitions()<CR>', opts)
	...
	keymap(bufnr, "n", "gl", '<cmd>lua require("telescope.builtin").diagnostics()<CR>', opts)
...
function M.config()
	...
	wk.add({
        ...
        {"<leader>lh", "<cmd>lua require('base.lspconfig').toggle_inlay_hints()<cr>", desc="Hints" },
	...
	for _, server in pairs(servers) do
		...
		local require_ok, settings = pcall(require, "base.lspsettings." .. server)
		if require_ok then
